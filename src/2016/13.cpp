...
struct Point {
  ...
  constexpr Point operator+(int a) const {
    return {x + a, y + a};
  }
  constexpr Point operator-(int a) const {
...
struct std::hash<Point> {
  std::size_t operator()(const Point& p) const noexcept {
    ...
    constexpr auto half_width{std::numeric_limits<Point::UInt>::digits / 2};
    const auto lhs{p.x << half_width};
    const auto rhs{p.y & ((Point::UInt{1} << half_width) - 1)};
    return std::hash<std::size_t>{}(lhs | rhs);
  }
};

using Points = std::vector<Point>;
using Distances = std::unordered_map<Point, int>;

class Dijkstra {
 public:
  explicit Dijkstra(Point begin, Point end, int step_limit)
  ...
  bool has_visited(const Point& p) const {
  ...
  void visit(const Point& src, const Point& dst) {
  ...
  Distances all_distances() const {
    return dist | views::transform([](auto&& item) -> std::pair<Point, int> {
             auto&& [k, v] = item;
             return {k - 1, v};
           })
           | ranges::to<Distances>();
...
constexpr bool is_wall(const Point& p, const int designer_num) {
...
Distances find_shortest_paths(
  ...
  for (const Point& target : targets) {
    ...
    while (search) {
      Point src{search.pop_next()};
      for (const auto& [dx, dy] : adj_deltas) {
        if (Point dst{src.x + dx, src.y + dy};
...
auto find_part2(const int designer_num, const int step_limit) {
  ...
  for (std::size_t x{0}; x <= step_limit + 1; ++x) {
    for (std::size_t y{0}; y <= step_limit + 1; ++y) {
