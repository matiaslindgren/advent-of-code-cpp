...
Options parse_password(const std::string& line) {
...
std::optional<long> explore_step(auto& q, auto& seen) {
...
auto explore(const auto& program) {
  ...
  for (std::deque q{SearchState{.ic = IntCode(program)}}; not q.empty(); q.pop_front()) {
    if (auto password{explore_step(q, seen)}) {
