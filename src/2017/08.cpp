...
auto search(const auto& statements) {
  std::unordered_map<std::string, int> memory;
  int part2{};
  for (auto&& [ins, cond] : statements) {
    if (cond.comp(memory[cond.key], cond.value)) {
      memory[ins.key] = ins.op(memory[ins.key], ins.value);
      part2 = std::max(part2, memory[ins.key]);
    }
  }
  auto part1{ranges::max(views::values(memory))};
...
int main() {
  ...
  const auto [part1, part2]{search(statements)};
